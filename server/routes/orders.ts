...
import { z } from "zod";
import { insertUserAddressSchema, insertUserSchema } from "@shared/schema";
...
export function createOrdersRouter({ storage }: Pick<RouteDependencies, "storage">) {
  const router = Router();

  router.post("/orders", async (req: SessionRequest, res) => {
