...
export function createAuthRouter({ storage, otpService }: RouteDependencies) {
  const router = Router();

  router.post("/auth/login", async (req: SessionRequest, res) => {
  ...
  router.post("/auth/logout", async (req: SessionRequest, res) => {
  ...
  router.get("/auth/orders", async (req: SessionRequest, res) => {
    if (!req.session.userId || req.session.userRole !== "buyer") {
  ...
  router.get("/auth/addresses", async (req: SessionRequest, res) => {
    if (!req.session.userId || req.session.userRole !== "buyer") {
      return res.status(401).json({ message: "Authentication required" });
    }
  ...
  router.get("/auth/addresses/last", async (req: SessionRequest, res) => {
    if (!req.session.userId || req.session.userRole !== "buyer") {
      return res.status(401).json({ message: "Authentication required" });
    }
  ...
  router.post("/auth/addresses", async (req: SessionRequest, res) => {
    if (!req.session.userId || req.session.userRole !== "buyer") {
      return res.status(401).json({ message: "Authentication required" });
    }
  ...
  router.put("/auth/addresses/:id/preferred", async (req: SessionRequest, res) => {
    if (!req.session.userId || req.session.userRole !== "buyer") {
      return res.status(401).json({ message: "Authentication required" });
    }
  ...
  router.delete("/auth/addresses/:id", async (req: SessionRequest, res) => {
    if (!req.session.userId || req.session.userRole !== "buyer") {
      return res.status(401).json({ message: "Authentication required" });
    }
  ...
  router.post("/auth/send-otp", async (req: SessionRequest, res) => {
    try {
  ...
  router.post("/auth/verify-otp", async (req: SessionRequest, res) => {
    try {
  ...
  router.post("/auth/login-password", async (req: SessionRequest, res) => {
    try {
