...
export function createInfluencerRouter({ storage }: Pick<RouteDependencies, "storage">) {
  const router = Router();
  ...
  router.patch("/influencers/:id/deactivate", async (req, res) => {
    try {
  ...
  router.post("/influencer/login", async (req: SessionRequest, res) => {
    try {
  ...
  router.post("/influencer/logout", (req: SessionRequest, res) => {
    req.session.influencerId = undefined;
    req.session.userRole = undefined;
    res.json({ message: "Logged out successfully" });
  });

  router.get("/influencer/me", async (req: SessionRequest, res) => {
