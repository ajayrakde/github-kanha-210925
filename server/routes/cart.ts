...
export function createCartRouter({ storage }: Pick<RouteDependencies, "storage">) {
  const router = Router();

  router.get("/cart", async (req: SessionRequest, res) => {
    try {
  ...
  router.post("/cart/add", async (req: SessionRequest, res) => {
    try {
  ...
  router.patch("/cart/:productId", async (req: SessionRequest, res) => {
    try {
  ...
  router.delete("/cart/:productId", async (req: SessionRequest, res) => {
    try {
  ...
  router.delete("/cart/clear", async (req: SessionRequest, res) => {
    try {
