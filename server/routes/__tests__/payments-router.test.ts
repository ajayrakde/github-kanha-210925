...
describe("payments router", () => {
  ...
  describe("POST /api/payments/token-url", () => {
    ...
    it("invalidates and recreates the token URL when the cached attempt expired", async () => {
      ...
      const tokenUrlKey = computeKey("tenant-a", "order-123", amountMinor, "INR");
      ...
      idempotencyCache.set(`phonepe_token_url:${tokenUrlKey}`, expiredPayload);
      ...
      expect(invalidateKeyMock).toHaveBeenCalledWith(tokenUrlKey, "phonepe_token_url");
      expect(mockPaymentsService.createPayment).toHaveBeenCalledTimes(1);
      expect(mockPhonePePollingWorker.registerJob).toHaveBeenCalledTimes(1);
    ...
    it("derives a deterministic idempotency key from the tenant, order, and amount", async () => {
      ...
      const tokenUrlKey = computeKey("tenant-a", "order-123", expectedAmountMinor, "INR");
      const paymentKey = `${tokenUrlKey}::create_payment`;

      expect(mockPaymentsService.createPayment).toHaveBeenCalledWith(
        expect.objectContaining({ idempotencyKey: paymentKey, orderAmount: expectedAmountMinor }),
        "tenant-a",
        "phonepe"
      );
