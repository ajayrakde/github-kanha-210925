...
export function createPaymentsRouter(requireAdmin: RequireAdminMiddleware) {
  ...
  /**
   * Test endpoint to create a dummy Cashfree order and payment
   * POST /api/payments/test/cashfree-dummy-order
   * This endpoint creates a test order and initiates Cashfree payment to test webhooks
   */
  router.post('/test/cashfree-dummy-order', async (req: SessionRequest, res) => {
    try {
      const tenantId = resolveTenantId(req);
      const environment = (process.env.NODE_ENV === 'production' ? 'live' : 'test') as Environment;
      ...
      const orderAmount = 10000; // Rs. 100.00 in paise
      const orderId = `test-order-${Date.now()}`;

      console.log(`[Test] Creating dummy Cashfree order: ${orderId}`);
      ...
      const paymentResult = await cashfreeAdapter.createPayment({
        orderId,
        orderAmount,
        currency: 'INR',
        customer: {
        ...
        successUrl: `${req.protocol}://${req.get('host')}/test-payment-success?orderId=${orderId}`,
        failureUrl: `${req.protocol}://${req.get('host')}/test-payment-failed?orderId=${orderId}`,
      });

      console.log(`[Test] Cashfree order created successfully:`, paymentResult);

      // Get the checkout URL for testing
      const checkoutBaseUrl = environment === "live"
        ? "https://payments.cashfree.com/order"
        : "https://sandbox.cashfree.com/pg/view/order";
      
      const checkoutUrl = paymentResult.providerData?.paymentSessionId
        ? `${checkoutBaseUrl}/${paymentResult.providerOrderId}/${paymentResult.providerData.paymentSessionId}`
        : undefined;

      // Return payment details
      res.json({
        success: true,
        message: 'Test Cashfree order created successfully',
        data: {
          ...
          environment,
          checkoutUrl,
          paymentSessionId: paymentResult.providerData?.paymentSessionId,
          webhookUrl: `${req.protocol}://${req.get('host')}/api/payments/webhook/cashfree`,
          instructions: [
            '1. Open the checkout URL to complete payment',
            '2. Use Cashfree test credentials to simulate payment',
            '3. Check server logs for webhook reception',
            '4. Webhook will be sent to the webhookUrl shown above'
          ]
        }
      });

    } catch (error) {
      console.error('[Test] Failed to create dummy Cashfree order:', error);
      res.status(500).json({
