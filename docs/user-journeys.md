...
5. **Checkout / Place Order**
   ...
   - Refund creation now enforces captured-vs-refunded guardrails, persists cumulative refund totals, and exposes masked refund identifiers through `/api/payments/order-info/:orderId` so post-checkout surfaces can show settled refund evidence without revealing raw UTRs.
