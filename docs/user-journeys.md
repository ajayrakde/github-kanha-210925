...
5. **Checkout / Place Order**
  ...
  - The payment page now renders a unified UPI payment widget that displays canonical PhonePe metadata (merchant name, VPA, QR payloads, normalized amounts) and triggers app-intent launches directly while coordinating polling through a shared hook so Cashfree collect requests and PhonePe retries keep the status strip in sync.
