...
5. **Checkout / Place Order**
   ...
   - PhonePe hosted checkout now powers UPI payments: `/api/payments/token-url` issues a `PAY_PAGE` request with `payPageType: 'REDIRECT'`, returning PhonePe's hosted URL so the gateway picks the right surface (intent on mobile, QR on desktop) without exposing raw instrument types to shoppers.
   - `/api/payments/token-url` still cross-checks the stored order amount/currency before creating a PhonePe session, rejecting tampered payloads with an audit trail so hosted launches cannot be spoofed by modified client requests.
   - Checkout keeps a single “Pay via UPI” button that simply hits `/api/payments/token-url` and then redirects the browser to PhonePe. This matches PhonePe’s recommendation for web, keeps the UI friendly, and leans on the backend/webhook pair as the source of truth before clearing the cart.
   - After PhonePe redirects the buyer back, the thank-you page first pings `/api/payments/phonepe/return` to log a "processing" event without touching order state, then begins polling `/api/payments/order-info/:orderId`. Once the backend reports `paid` the page clears the shopper's cart and swaps the badge to "Paid".
   ...
   - The PhonePe adapter still requests a deterministic idempotency key per order+amount so rapid double-submissions get the same token URL and expired attempts are safely invalidated before retrying.
