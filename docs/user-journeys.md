...
5. **Checkout / Place Order**
   ...
   - The make-payment screen now centralizes UPI intent and QR options in an accessible widget: mobile defaults to the intent tab, larger screens default to QR, copy buttons expose the deep link/VPA/UTR with transient feedback, and the status bar mirrors backend state without altering the checkout API contracts.
   ...
   - The payment screen now consumes a dedicated `useUpiPaymentState` hook that maps backend `COMPLETED/FAILED/PENDING` results into the widget's `success/failed/expired/awaiting` badges, memoizes the canonical UPI deep link + QR (with a pulsing placeholder when QR generation fails), and wraps clipboard writes so buyers can safely copy either the dynamic UPI URI or the gateway checkout URL without triggering UI regressions when the browser blocks clipboard access.
...
4. **Orders & Analytics**
   ...
   - The sales trends card now renders a lightweight two-column `SalesTrendsCard` component: the left rail exposes the time range dropdown (week through year) and the data toggles for **Sales** and **Orders**, while the right rail houses the dual-axis Recharts plot with aggregation buttons (**D/W/M/Q**) that auto-adjust to the active range (daily disabled for yearly windows). Enabling the splitting control lets admins spotlight the top/bottom N products for the single active metric with clustered bars that can be toggled from the legend without dismounting the series, and the cumulative line hides automatically so only the product bars remain. The popular products list still highlights the top three performers.
